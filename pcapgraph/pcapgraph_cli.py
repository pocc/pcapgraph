...
USAGE:
  ::
    ...
    pcapgraph [-abdeimpsuwxy23] (<file>)... [--output <format>]...
    pcapgraph (-V | --version)
    pcapgraph (-h | --help)
...
DESCRIPTION:
    ...
    the temporal overlap of packets. Set operations will hash packet hex to
    find same/different ones. The default behavior for output is a graph
    (hence the name).
...
OPTIONS:
    FILTERING OPTIONS
          Apply these operations prior to performing set operations
    ...
    BASIC SET OPERATIONS:
      ...
      -y, --symmetric-difference
                            Packets unique to each packet capture.

    COMPOUND SET OPERATIONS:
      -b, --bounded-intersection
                            Find the first and last common packets between
                            packet captures by timestamp. Excise packets
                            between the first and last packet in each capture
                            and output each as a separate new pcap.
      -e, --inverse-bounded
    ...
    OUTPUT OPTIONS:
      ...
      -w, --wireshark       Open pcaps in Wireshark after creation.
...
SET OPERATIONS:
    ...
    1. Find all unique packets by their ASCII hexdump value (and hash).
...
SEE ALSO:
    ...
    tcpreplay-edit (http://tcpreplay.synfin.net/tcpreplay-edit.html):
        Replay and modify pcap files.
...
import pcapgraph._version as _version
import pcapgraph.pcap_math as sets
import pcapgraph.wireshark_io as ws_utils
import pcapgraph.plot_graph as graph
import pcapgraph.pcap_io as pcap_io


def init_cli(args):
    """Start PcapGraph CLI."""
    args['<file>'] = sorted(ws_utils.parse_filenames(args['<file>']))
    if args['--version']:
        print_version()
    # Categorize output formats/options
    strip_options = get_strip_options(args)
    output_options = get_output_options(args)

    pcaps_frame_dict = {}
    generated_files = []
    if requires_set_operations(args):
        pcap_obj = sets.PcapMath(args['<file>'], strip_options)
    ...
    if requires_graph_operations(args):
        output_files(pcaps_frame_dict, args['<file>'], generated_files,
...
def get_docstring():
    """Reformat the docstring so that RST signals are removed."""
    return re.sub(r' *:: *\n\n|`|\*', '', __doc__)


def print_version():
    """Print versions and exit."""
    print('Pcapgraph', _version.__version__, '\n')
    print('python', sys.version)
    print(ws_utils.get_tshark_version())
    sys.exit()
...
def get_output_options(args):
    ...
    output_option_list = [
        '--anonymize', '--show-packets', '--exclude-empty', '--wireshark',
        '--plot'
    ]
...
def requires_set_operations(args):
    ...
    has_set_ops = bool(get_set_operations(args))
    pcap_fmts = {'pcap', 'pcapng'}.intersection(args['--output'])
    return has_set_ops or args['--wireshark'] or \
        args['--most-common-frames'] or pcap_fmts


def requires_graph_operations(args):
    """Return whether the user's inputs require graph operations to be used."""
    has_graph_ops = bool(get_set_operations(args))
    image_fmts = graph.get_matplotlib_fmts().intersection(args['--output'])
    return has_graph_ops or args['--wireshark'] or \
        args['--most-common-frames'] or image_fmts
...
def output_files(pcap_data, input_files, new_files,
                 output_formats, output_options):
    """Save the specified file with the specified format.

    Args:
        save_format (str): Extension of file to be saved.
        pcap_data (dict): All packets, where key is pcap filename/operation.
    ...
    """
    pcap_timestamps = {}
    for file in pcap_data:
        pcap_timestamps[file] = pcap_data[file]['timestamps']
    pcap_metadata = get_pcap_metadata(pcap_timestamps, input_files, new_files)

    if '--exclude-empty' in output_options:
        pcap_metadata = remove_empty_outputs(pcap_metadata, input_files)
    graph.generate_graph(pcap_data, pcap_metadata, [],
    ...
    if '--plot' in output_formats:
        ...
        for output_format in output_formats:
            graph.export_graph(input_files, output_format)


def get_pcap_metadata(pcap_timestamps, input_files, new_files):
    """Get metadata about pcaps.

    Args:
        pcap_timestamps (dict): Frames and timestamps by pcap
        new_files (list): User inputted filenames
    """
    pcap_metadata = ws_utils.get_pcap_info(input_files)
    for pcap in new_files:
        float_timestamps = []
        if pcap_timestamps[pcap]:
            for timestamp in pcap_timestamps[pcap]:
                float_timestamps.append(pcap_io.get_ts_as_float(timestamp))
        pcap_name = os.path.splitext(pcap)[0]
        if float_timestamps:
            pcap_metadata[pcap_name] = {
                'packet_count': len(pcap_timestamps[pcap]),
                'pcap_start': min(float_timestamps),
                'pcap_end': max(float_timestamps)
            }
    return pcap_metadata


def remove_empty_outputs(pcap_metadata, input_files):
    """Remove output files if they have no packets."""
    new_pcap_metadata = {}
    for pcap in pcap_metadata:
        is_input_file = pcap in input_files
        has_packets = pcap_metadata[pcap]['packet_count'] > 0
        if is_input_file or has_packets:
            new_pcap_metadata[pcap] = pcap_metadata[pcap]
