...
def draw_graph(pcaps_dicts, set_pcap_json, save_fmts):
    """Draw a graph using matplotlib and numpy.

    Args:
        pcaps_dicts (dict): All packets organized by packet capture filename.
        set_pcap_json (dict): Any packet captures generated by set operations.
    ...
    for save_format in save_fmts:
        if save_format == 'txt':
            output_text = make_text_not_war(pcaps_dicts)
            print(output_text)
            ...
            start_times, end_times, pcap_names = get_graph_vars(pcaps_dicts)

            generate_graph(pcap_names, start_times, end_times)
            export_graph(pcaps_dicts, pcap_names, save_format)
